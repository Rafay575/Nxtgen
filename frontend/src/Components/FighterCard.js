...
const FighterCard = () => {
  ...
  return (
    <>
      <div className='bg-black py-5 text-white text-center' id='home'>
        ...
        <div className="flex flex-col md:flex-row items-center justify-center md:justify-around">
          {athletes.map((athlete, index) => (
            <div key={index} className="relative mb-8 group cursor-pointer" onClick={() => openModal(athlete)}>
      ...
      >
        {selectedAthlete && <AthleteDetails athlete={selectedAthlete} closeModal={closeModal} />}
      </Modal>
    </>
  );
};
